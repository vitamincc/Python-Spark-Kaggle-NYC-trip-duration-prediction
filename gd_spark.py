...
def rmse_mae_gd(trainset, testset):
	model_gd = LinearRegressionWithSGD.train(trainset, iterations=50, step=0.00000001)
	predicted = testset.map(lambda p: (p.label, model_gd.predict(p.features)))
	RMSE = sqrt(predicted.map(lambda vp: (vp[0] - vp[1])**2).reduce(lambda x, y: x + y) / predicted.count())
	MAE = predicted.map(lambda vp: abs(vp[0] - vp[1])).reduce(lambda x, y: x + y) / predicted.count()
...
rmse_mae_gd(traindata, testdata)
